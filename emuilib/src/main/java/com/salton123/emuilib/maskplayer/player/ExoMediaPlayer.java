...
import com.salton123.emuilib.util.Repeater;
...
//        Log.e("onPlayerStateChanged: playWhenReady = " + playWhenReady + ", playbackState = " + playbackState);
            if (lastReportedPlayWhenReady != playWhenReady || lastReportedPlaybackState != playbackState) {
                if (mIsBuffering) {
                    switch (playbackState) {
                        ...
                        case Player.STATE_READY:
                            if (mPlayerEventListener != null) {
                                mPlayerEventListener.onInfo(MediaPlayer.MEDIA_INFO_BUFFERING_END, mInternalPlayer.getBufferedPercentage());
                            }
                            mIsBuffering = false;
                            break;
                    }
                ...
                switch (playbackState) {
                    case Player.STATE_BUFFERING:
                        if (mPlayerEventListener != null) {
                            mPlayerEventListener.onInfo(MediaPlayer.MEDIA_INFO_BUFFERING_START, mInternalPlayer.getBufferedPercentage());
                        }
                        mIsBuffering = true;
                        break;
                    case Player.STATE_READY:
    ...
    public void onRenderedFirstFrame(Surface surface) {
        if (mPlayerEventListener != null && mIsPrepareing)
            mPlayerEventListener.onInfo(MediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START, 0);
    }
