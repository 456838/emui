package com.salton123.emuilib.niceplayer.player;
...
public class NiceVideoPlayer extends FrameLayout
    ...
    public void setSpeed(float speed) {
        if (mMediaPlayer instanceof IjkMediaPlayer) {
            ...
            MLog.debug("只有IjkPlayer才能设置播放速度");
        }
    }
    ...
    public void start() {
        if (mCurrentState == STATE_IDLE) {
            ...
            MLog.debug("NiceVideoPlayer只有在mCurrentState == STATE_IDLE时才能调用start方法.");
        }
    }
    ...
    public void restart() {
        if (mCurrentState == STATE_PAUSED) {
            ...
            mController.onPlayStateChanged(mCurrentState);
            MLog.debug("STATE_BUFFERING_PLAYING");
        } else if (mCurrentState == STATE_COMPLETED || mCurrentState == STATE_ERROR) {
            mMediaPlayer.reset();
            openMediaPlayer();
            start();
        } else {
            MLog.debug("NiceVideoPlayer在mCurrentState == " + mCurrentState + "时不能调用restart()方法.");
        }
    }
    ...
    public void pause() {
        ...
        if (mCurrentState == STATE_BUFFERING_PLAYING) {
            ...
            mController.onPlayStateChanged(mCurrentState);
            MLog.debug("STATE_BUFFERING_PAUSED");
        }
    }
    ...
    private void openMediaPlayer() {
        ...
        try {
            ...
            mController.onPlayStateChanged(mCurrentState);
            MLog.debug("STATE_PREPARING");
        } catch (IOException e) {
            e.printStackTrace();
            MLog.error("打开播放器发生错误", e);
        }
    }
    ...
    private IMediaPlayer.OnVideoSizeChangedListener mOnVideoSizeChangedListener
        ...
        public void onVideoSizeChanged(IMediaPlayer mp, int width, int height, int sar_num, int sar_den) {
            ...
            MLog.debug("onVideoSizeChanged ——> width：" + width + "， height：" + height);
        }
    };
        ...
        public boolean onInfo(IMediaPlayer mp, int what, int extra) {
            ...
            if (what == IMediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START) {
                ...
                if (mCurrentState == STATE_PAUSED || mCurrentState == STATE_BUFFERING_PAUSED) {
                    mCurrentState = STATE_BUFFERING_PAUSED;
                    MLog.debug("onInfo ——> MEDIA_INFO_BUFFERING_START：STATE_BUFFERING_PAUSED");
                } else {
                    mCurrentState = STATE_BUFFERING_PLAYING;
                    MLog.debug("onInfo ——> MEDIA_INFO_BUFFERING_START：STATE_BUFFERING_PLAYING");
                }
                mController.onPlayStateChanged(mCurrentState);
                ...
                if (mTextureView != null) {
                    ...
                    MLog.debug("视频旋转角度：" + extra);
                }
            } else if (what == IMediaPlayer.MEDIA_INFO_NOT_SEEKABLE) {
                MLog.debug("视频不能seekTo，为直播视频");
            } else {
                MLog.debug("onInfo ——> what：" + what);
            }
            return true;
        }
    };
    ...
    public void updatePlayUrl(String url, Map<String, String> headers) {
        releasePlayer();
        setUp(url, headers);
    }
